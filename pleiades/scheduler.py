...
The background runner (`start_scheduler_thread`) wakes once a minute, fires
any due task via a job-launcher callback the caller supplies (this module has
no import-time dependency on the webui server — see `set_job_launcher`), and
recomputes next_run. One-time (`fire_at`) tasks disable themselves after
firing; recurring (`cron`) tasks compute their next occurrence and keep going.
"""
...
class ScheduledTaskManager:
    ...
    def _save(self, data: dict) -> None:
        _tasks_json().write_text(json.dumps(data, indent=2), encoding="utf-8")
