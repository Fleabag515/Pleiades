...
def runtime_build_engine() -> None:
    ...
    if _has("nvcc"):
        ...
        backend = "vulkan"
    if backend != "cpu":
        backend_flags = [f"-DPLEIADES_ENGINE_{'CUDA' if backend == 'cuda' else 'VULKAN'}=ON",
                         "-DPLEIADES_ENGINE_CUDA=OFF"] if backend == "vulkan" else []
    if backend == "cuda":
        backend_flags = ["-DPLEIADES_ENGINE_CUDA=ON"]

    console.print(f"[dim]Configuring engine ({backend}) in {build_dir} ...[/dim]")
    for cmd in (["cmake", "-S", str(src), "-B", str(build_dir), *backend_flags],
