...
void ContextGovernor::create_ctx(int n_ctx) {
    ...
    // Phase 2 — KV placement: offload_kqv=false keeps the K/V cache in host
    // RAM while compute stays on the GPU (the spill configuration).
    params.offload_kqv = !kv_on_cpu_active_;

    // Phase 2 — elastic RoPE: past the trained context, engage YaRN with a
...
ContextGovernor::GrowResult ContextGovernor::grow_preserving(int requested) {
    ...
    // K/V cost of the TARGET context straight from the model geometry
    // (n_layer x n_kv_head x head_dims x element size). A residency-based
    // estimate (current state size extrapolated) reads zero on a fresh
    // context and would never spill the first, often largest, growth.
    const double est = static_cast<double>(kv_bytes_per_token()) * static_cast<double>(target);
    const double est_mib = est / (1024.0 * 1024.0);
    const bool kv_cpu_before = kv_on_cpu_active_;
    ...
    if (kv_on_cpu_active_ && cpu_budget_ && est > static_cast<double>(cpu_budget_)) {
        // The context was NOT recreated yet -- roll the spill flag back so the
        // governor's state stays consistent with the live (GPU-KV) context.
        kv_on_cpu_active_ = params_.kv_on_cpu = kv_cpu_before;
        throw std::runtime_error(
    ...
    const int old_ctx = n_ctx_;
    const llama_pos pos_max_before = ctx_ ? llama_memory_seq_pos_max(llama_get_memory(ctx_), 0) : -1;
    create_ctx(target);
    r.n_ctx = n_ctx_;
    ...
    std::fprintf(stderr,
                 "[pleiades-engine] grow %d->%d: state=%zu bytes, pos_max before=%ld after=%ld (preserved=%d)\n",
                 old_ctx, target, state_sz, static_cast<long>(pos_max_before),
                 static_cast<long>(llama_memory_seq_pos_max(llama_get_memory(ctx_), 0)),
                 r.preserved ? 1 : 0);
    return r;
}
...
size_t ContextGovernor::kv_bytes_per_token() const {
    ...
    // (key_length + value_length) x sizeof(elem), GQA-aware — the same
    // formula hardware.py::kv_bytes_per_token uses on the Python side. The
    // geometry comes from the GGUF metadata (arch-namespaced keys, so read
    ...
    auto meta_u32 = [&](const char* suffix) -> long {
        char key[128];
        char buf[64];
    ...
    long kv_heads = meta_u32(".attention.head_count_kv");
    long key_len = meta_u32(".attention.key_length");
    long val_len = meta_u32(".attention.value_length");
...
std::optional<int> ContextGovernor::next_gear() const {
    ...
    // Past the ladder: keep doubling (unbounded when no pin is set).
    int g = n_ctx_;
    while (g <= n_ctx_) {
        g *= 2;
