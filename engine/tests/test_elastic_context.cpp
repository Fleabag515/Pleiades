// Phase E — elastic context: KV-preserving growth, auto-upshift, auto-YaRN,
// and the CPU spill.
//
...
int main(int argc, char** argv) {
    ...
    // -- 2. RAM budget exhaustion -> honest error, not truncation (fixture) -- //
    {
        ...
        // CPU budget is then exceeded too.
        rig.ctx.set_budgets(/*kv_budget_bytes=*/1024, /*cpu_budget_bytes=*/1024);
        rig.engine.complete("Once upon a time", /*n_predict=*/4);
        bool threw = false;
        try {
            (void)rig.engine.grow_context_to(2048);
    ...
    // 3a. grow_preserving within the trained context.
    {
    ...
    // 3b. auto-upshift from a request (ensure_capacity).
    {
    ...
    // 3c. GPU KV budget -> spill to host RAM (never truncate).
    {
        Rig rig(real, /*n_ctx=*/1024);
        ...
        // flips on near-ties. Contract asserted: the state survived the
        // device move (preserved) and the reused KV is actually served.
    ...
    // 3d. auto-upshift with a spill mid-request.
    {
        const std::string big = long_prompt(95);
        Rig rig(real, /*n_ctx=*/1024);
        rig.ctx.set_budgets(/*kv_budget_bytes=*/1 << 20, /*cpu_budget_bytes=*/512 << 20);
