...
function ApprovalCard({ approval, onApprove, onDeny, busy }: ApprovalCardProps): React.JSX.Element {
  return (
    <div className="mx-auto w-full max-w-3xl rounded-2xl border border-accent/40 bg-accent/10 p-4">
      <div className="flex items-center gap-2 text-sm font-medium text-ink-bright">
        <span className="text-accent">⚠</span>
        Wants to run a tool
      </div>
      <div className="mt-2 rounded-lg bg-bg-100 p-3 font-mono text-xs text-ink-dim">
      ...
      <div className="mt-3 flex justify-end gap-2">
        <button
          onClick={onDeny}
          disabled={busy}
          className="rounded-lg bg-bg-200 px-3 py-1.5 text-sm text-ink transition hover:bg-bg-300 disabled:opacity-50"
        >
          Deny
        </button>
        <button
