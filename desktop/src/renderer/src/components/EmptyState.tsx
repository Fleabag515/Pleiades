...
function EmptyState({ characterName, onNewChat }: EmptyStateProps): React.JSX.Element {
  return (
    <div className="flex h-full w-full flex-col items-center justify-center gap-4 px-6 text-center">
      ...
      <h1 className="text-xl font-semibold text-ink">Start a conversation</h1>
      ...
      <button
        onClick={onNewChat}
        className="mt-2 rounded-xl bg-accent px-4 py-2 text-sm font-medium text-white transition hover:bg-accent-hover"
      >
        New chat
      </button>
    </div>
